 ...
 */
export function pixelsToNormalisedGrayscale(data: Uint8ClampedArray, cols: number, rows: number): Float32Array {
  const n = cols * rows;
  const gray = new Float32Array(n);

  for (let i = 0; i < n; i++) {
    const p = i * 4;
    gray[i] = data[p + 3] < 128 ? 255 : 0.299 * data[p] + 0.587 * data[p + 1] + 0.114 * data[p + 2];
  }
 ...
 */
export function floydSteinberg(gray: Float32Array, cols: number, rows: number, threshold = 128): number[][] {
